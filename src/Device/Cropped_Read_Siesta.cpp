...
#include<fstream>
#include<chrono>
#include "Cropped_Read_Siesta.hpp"
#include<fstream>
...
Cropped_Read_Siesta::Cropped_Read_Siesta(device_vars& device_vars):Device(device_vars){};
...
void Cropped_Read_Siesta::create_csr_sparse_matrix(const double prune_threshold,
                                     const int no_u,
                                     const int ncomp,
...
int Cropped_Read_Siesta::read_siesta_reciprocal_matrix(const char *fn,
...
void Cropped_Read_Siesta::build_Hamiltonian(){
  std::ifstream inFile;
  ...
  */
  ...
  parameters().DIM_    = DIM-4*nk;
  parameters().SUBDIM_ = DIM-4*nk;  
  parameters().C_      = 0;
  parameters().W_      = 1;
  parameters().LE_     = DIM-4*nk;
...
void Cropped_Read_Siesta::trimBlocks(SpMatrixXcp& Hc, const SpMatrixXcp& Hc_tmp, int nk, int block_size) {
    int new_block_size = block_size - 4;
    int new_dim = nk * new_block_size;

    // New sparse matrix
    //Eigen::SparseMatrix<double> Hc_(new_dim, new_dim);
    std::vector<Eigen::Triplet<std::complex<double>>> triplets;
    triplets.reserve(Hc_tmp.nonZeros());
    ...
    for (int k = 0; k < nk; ++k) {
        ...
        for (int j = 0; j < new_block_size; ++j) {
            for (SpMatrixXcp::InnerIterator it(Hc_tmp, old_block_col_start + j);
                 it; ++it) {
                indexType i = it.row();
                indexType j_global = it.col();

                // Keep only entries within the trimmed block
                if (i >= old_block_row_start && i < old_block_row_start + new_block_size &&
                    j_global >= old_block_col_start && j_global < old_block_col_start + new_block_size) {
                    indexType new_i = new_block_row_start + (i - old_block_row_start);
                    indexType new_j = new_block_col_start + (j_global - old_block_col_start);
...
void Cropped_Read_Siesta::vel_op_x (type vec[], type p_vec[]){
...
void Cropped_Read_Siesta::vel_op_y (type vec[], type p_vec[]){
  int Dim = this->parameters().DIM_;
...
void Cropped_Read_Siesta::H_ket ( type* vec, type* p_vec ){
  H_ket(vec, p_vec, damp_op(), dis());
};



void Cropped_Read_Siesta::H_ket ( type* vec, type* p_vec, r_type* dmp_op, r_type* dis_vec) {
  int Dim = this->parameters().DIM_,
...
void Cropped_Read_Siesta::update_cheb ( type vec[], type p_vec[], type pp_vec[]){
  update_cheb ( vec, p_vec, pp_vec, damp_op(), NULL);
};

  
void Cropped_Read_Siesta::update_cheb ( type vec[], type p_vec[], type pp_vec[], r_type damp_op[], r_type*){
...
void Cropped_Read_Siesta::damp ( r_type damp_op[]){
...
void Cropped_Read_Siesta::setup_velOp(){
  std::ifstream inFile;
  std::string run_dir  = parameters().run_dir_,
    filename = parameters().filename_;
...
void Cropped_Read_Siesta::update_dis ( r_type dis_vec[], r_type damp_op[]){
