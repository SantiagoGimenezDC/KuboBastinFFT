...
void Kubo_solver_filtered::Sea_FFTs_imag  (r_type E_points[], std::complex<r_type>** bras_re, std::complex<r_type>** bras_im, std::complex<r_type>** kets_re, std::complex<r_type>** kets_im,  type* r_data, int s){
...
# pragma omp critical
   ...
   for(int l = l_start; l < l_end; l++){
     ...
     for(int j = 0; j < nump; j++){
	   ...
	   ( imag(kets_re_dft(j))         + im * imag(kets_im_dft(j)) ) + //Re(G(k)) * G(k)+

	   ( imag(conj( bras_re_dft(j) )) - im * imag(conj(bras_im_dft(j) ) ) )  *
	   ( real( kets_re_dft(j) ) + im * real( kets_im_dft(j) ) ) ;   //G(k) * Re(G(k))
	

       //Here: w(k) += (dG(k)) * Re(G(k)) - Re(G(k)) * (dG(k))
         w[j] += ( real(conj( D_bras_re[j] ) )   - im * real(conj( D_bras_im[j] ) ) )    *
	         ( real( kets_re_dft(j) ) + im * real( kets_im_dft(j) ) ) - //dG(k) * Re(G(k))-	  

	         ( real( bras_re_dft(j) ) - im * real( bras_im_dft(j) ) )  *
	   ( real(D_kets_re[j] )           + im * real(D_kets_im[j] ) ) ; //Re(G(k)) * dG(k)
       }

       if( sign[j] == -1 ){
       //Here: p(k) += Re(G(k)) * G(k) + G(k) * Re(G(k)).
         p[j] +=  ( real( bras_re_dft(j) ) - im * real( bras_im_dft(j) )) *
	   ( imag(conj(kets_re_dft(j) ) )   + im * imag(conj(kets_im_dft(j) ) ) )  + //Re(G(k)) *G(k)+
	   
	   ( imag(bras_re_dft(j) )         - im * imag( bras_im_dft(j) ) )        *
	          ( real( kets_re_dft(j) ) + im * real( kets_im_dft(j) ) ) ;    //G(k)Re(G(k))
	

         //Here: w(k) += (dG(k)) * Re(G(k)) - Re(G(k)) * (dG(k))
         w[j] += ( real(D_bras_re[j])           - im * real(D_bras_im[j]) )           *
	         ( real( kets_re_dft(j) ) + im * real( kets_im_dft(j) ) ) - //dG(k)Re(G(k))

	         ( real( bras_re_dft(j) ) - im * real( bras_im_dft(j) ) ) *
	   ( real ( conj( D_kets_re[j]) )    + im * real(conj( D_kets_im[j] ) ) ); //Re(G(k))dG(k)
