...
void Kubo_solver_filtered::Sea_FFTs_doubleBuffer_imag  (r_type E_points[],
...
# pragma omp critical
   ...
   for(int l = l_start; l < l_end; l++){
	 ...
	 ( imag(kets_re_dft(j) )   + im *       imag(kets_im_dft(j)) ) + //G(k)+
 	  
	 ( imag(conj( bras_re_dft(j) )) - im * imag(conj( bras_im_dft(j)) ) ) * //G(k)
	         ( real( kets_re_dft(j) ) + im * real( kets_im_dft(j) ) );   //Re(G(k))
	

       //Here: w(k) += (dG(k)) * Re(G(k)) - Re(G(k)) * (dG(k))
       w[j] += ( real( conj( d_bras_re_dft(j) ) ) - im * real(conj( d_bras_im_dft(j) ) ) )  * //dG(k)
		 ( real( kets_re_dft(j)   ) + im * real( kets_im_dft(j) ) ) - //Re(G(k))
	  
		 ( real( bras_re_dft(j) )   - im * real( bras_im_dft(j) ) )  *  //Re(G(k))
	 ( real(d_kets_re_dft(j) )         + im * real( d_kets_im_dft(j) ) ); //dG(k)
       }

       if( sign[j] == -1 ){
       //Here: p(k) += Re(G(k)) * G(k) + G(k) * Re(G(k)).
       p[j] += ( real( bras_re_dft(j) ) - im * real( bras_im_dft(j) ) ) *  //Re(G(k)) *
	 ( imag( conj( kets_re_dft(j) ) ) + im * imag(conj( kets_im_dft(j) ) ) ) + //G(k)+
 	  
	 ( imag( bras_re_dft(j) )   - im * imag( bras_im_dft(j) )  ) * //G(k)
	         ( real( kets_re_dft(j) ) + im * real( kets_im_dft(j) ) );   //Re(G(k))
	

         //Here: w(k) += (dG(k)) * Re(G(k)) - Re(G(k)) * (dG(k))
       w[j] += (  real( d_bras_re_dft(j) )  - im *  real( d_bras_im_dft(j) )  )  * //dG(k)
		 ( real( kets_re_dft(j)   ) + im * real( kets_im_dft(j) ) ) - //Re(G(k))
	  
		 ( real( bras_re_dft(j) )   - im * real( bras_im_dft(j) ) )  *  //Re(G(k))
	 ( real( conj( d_kets_re_dft(j) ))         + im * real(conj ( d_kets_im_dft(j) ) ) ); //dG(k)
